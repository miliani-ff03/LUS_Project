"""
Video-Level Clustering Metrics Evaluation.

Computes clustering quality metrics comparing VAE cluster assignments
against LUS scores at the video level.

Usage:
    python -m video_level.clustering_metrics
    python video_level/clustering_metrics.py --cluster-table path/to/table.json
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Compute video-level clustering quality metrics")
    parser.add_argument(
        "--cluster-table", "-c",
    ...
    parser.add_argument(
        "--output", "-o",
        default=str(RESULTS_DIR / "video_clustering_metrics.csv"),
...
def main():
    ...
    if data_level != 'video':
        print("Warning: This script is designed for video-level data.")
    
    # Map video IDs to scores
    print("Mapping video IDs to scores...")
    cluster_df['Score'] = cluster_df['video_id'].apply(lambda vid: find_score(vid, df_metadata))
